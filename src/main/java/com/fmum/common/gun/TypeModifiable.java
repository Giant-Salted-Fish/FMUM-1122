...
public abstract class TypeModifiable extends TypePaintable
{
	protected static final AttachableSlot[] DEF_SLOTS = { };
	
	public static final LocalTypeFileParser<TypeModifiable>
		parser = new LocalTypeFileParser<>(TypePaintable.parser);
	static
	{
		
		// Attachable slots and default attachments TODO
		parser.addKeyword("AttachableSlots", (s, t) -> t.slots = AttachableSlot.parse(s, 1));
	 ...
	 */
	public AttachableSlot[] slots = DEF_SLOTS;
	
	protected TypeModifiable(String name, String contentPackName) { super(name, contentPackName); }
	
	public static final class AttachableSlot
	{
		private static final TreeSet<String> DEF_LIST = new TreeSet<>();
		
		/**
		 * Relative position of the tail of this slot on modifiable
		 */
		public float
			x = 0F,
			y = 0F,
		...
		public void applyModelScale(float s)
		{
			this.x *= s;
			this.y *= s;
			this.z *= s;
		...
		public boolean isAllowed(TypeModifiable type)
		{
			return(
		...
		private static final LinkedList<AttachableSlot> slots = new LinkedList<>();
		synchronized public static AttachableSlot[] parse(String[] split, int cursor)
		{
			// A error may occurred in last parse and leaving elements in buffer list.
			// Clear it before we start to use it
			slots.clear();
			
			AttachableSlot slot = new AttachableSlot();
			for(int i = cursor; i < split.length; ++i)
				switch(split[i])
				{
				case "|":
					slots.add(slot);
					slot = new AttachableSlot();
					cursor = i + 1;
					break;
			...
			AttachableSlot[] ret = slots.toArray(new AttachableSlot[slots.size()]);
			slots.clear();
			return ret;
		}
